...
public class UmbrellaGirlPaint_LeftAI : MonoBehaviour {
	...
	private static bool isBloodUmbrellaTaken;
	...
	void Update()
	{
		if (!isBloodUmbrellaTaken) {
			dialogHolder.Dialogs = new List<Dialog> { dialogHolder.TalkDialog ("牆上掛著一幅女人的畫像"),
				...
				dialogHolder.AskDialog ("搶走她的傘", "離開", new List<Dialog>{ dialogHolder.TalkDialog ("頭有點痛...") }),
				dialogHolder.TalkDialog ("......."),
				...
				dialogHolder.PickUpItemDialog ("獲得了 染血的雨傘", bloodUmbrella)
			};
		} else
			dialogHolder.Dialogs = new List<Dialog> { dialogHolder.TalkDialog ("牆上掛著一幅女人的畫像"),
				dialogHolder.TalkDialog ("似乎很憤怒地看著這邊的樣子...")
			};
	}
